...
from src.exceptions import BookingError, ValidationError
...
class TestBooking(unittest.TestCase):
    ...
    def test_booking_must_be_in_working_hours(self) -> None:
        with self.assertRaises(ValidationError):
            self.manager.book_haircut(
                ...
                self.chair.id,
                self.mirror.id,
                self._future_time(1, 7, 30),
            )

    def test_booking_must_be_in_future(self) -> None:
        with self.assertRaises(ValidationError):
            self.manager.book_haircut(
                self.client.id,
                self.hairdresser.id,
